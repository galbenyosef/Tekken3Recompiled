...
#include "host_osd_font.inl"

static char     s_msg[OSD_MAX_CHARS];
#include "tekken3_outfit_carousel.inl"
...
static SDL_Texture *s_sdl_outfit_tex[2];
static int s_sdl_outfit_tw[2],s_sdl_outfit_th[2];
#endif
...
void host_osd_draw_sdl(struct SDL_Renderer *renderer) {
    ...
    s_sdl_ren = renderer;
    {
        ...
        if (ui > 8) ui = 8;
        margin = 8 * ui;
        for(int player=0;player<2;player++)if(host_osd_outfit_image(player,&px,&w,&h)) {
            int x,y,dw,dh;
            host_osd_outfit_rect(player,lw,lh,w,h,&x,&y,&dw,&dh);
            /* Keep placement logical, but sample art at output resolution. */
            int output_w=lw,output_h=lh;
#if defined(PSX_SDL3)
            SDL_GetRenderOutputSize(renderer,&output_w,&output_h);
#else
            SDL_GetRendererOutputSize(renderer,&output_w,&output_h);
#endif
            double scale=(double)output_w/lw;
            if((double)output_h/lh<scale)scale=(double)output_h/lh;
            if(scale<=0)scale=1;
            int tw=(int)(dw*scale+.5),th=(int)(dh*scale+.5);
            if(tw<1)tw=1;
            if(th<1)th=1;
            if(host_osd_outfit_image_hires(player,&px,tw,th))
                sdl_blit_argb(renderer,&s_sdl_outfit_tex[player],&s_sdl_outfit_tw[player],
                    &s_sdl_outfit_th[player],px,tw,th,x,y,dw,dh);
        }
